...
- **项目面板**：在聊天右侧浏览和搜索文件、多标签预览与编辑，并查看真实 Git 变更和恢复项目快照。
...
项目会话可以打开右侧面板：支持文件树和文件名搜索，使用多标签预览 Markdown、HTML、代码、Diff、CSV、PDF、图片和文本，支持源码/预览切换、分屏编辑以及带外部修改检测的原子保存；“变更”页提供真实 Git 状态、Diff 和单文件撤销。面板宽度、折叠状态和当前页面均按项目保存。

每次助手回复完成后，桌面端会通过独立 Git 临时索引记录可恢复的 Tree 快照，不会改动用户暂存区；恢复前还会自动保存当前状态作为恢复点。Office 文件目前使用系统应用打开，后续再增加 DOCX/XLSX/PPTX 内置渲染。
