  ...
  factory: (require) => {
    ...
    function ConversationFiles({ matched, useSessions, sessionId }) {
      ...
      React.useEffect(() => {
        ...
        let cancelled = false;
        const frame = requestAnimationFrame(() => requestAnimationFrame(() => { if (!cancelled) window.dispatchEvent(new CustomEvent("dsh-desktop-project-file", { detail: { root, path: paths[paths.length - 1] } })); }));
        return () => { cancelled = true; cancelAnimationFrame(frame); };
      }, [root, matched?.turn, paths.join('\n')]);
      if (!root || !paths.length) return null;
      return h("div", { className: "dpp-conversation-files" }, h("span", null, `回复 ${matched.turn} 修改：`), ...paths.map((filePath) => h("button", { key: filePath, onClick: () => window.dispatchEvent(new CustomEvent("dsh-desktop-project-file", { detail: { root, path: filePath } })) }, filePath.split(/[\\/]/).pop())));
    }
    ...
    function Changes({ root, onOpen }) {
      ...
      return h("div", { className: "dpp-changes" }, error ? h("div", { className: "dpp-error" }, error) : null, h("div", { className: "dpp-section-title" }, h("strong", null, "本次对话变更（提交后仍保留）")), ...turnChanges.flatMap((group) => [h("div", { className: "dpp-turn-label", key: `turn-${group.turn}` }, `回复 ${group.turn}`), ...group.paths.map((filePath) => h("button", { className: "dpp-turn-file", key: `${group.turn}-${filePath}`, onClick: () => api.projectPanel.read(root, filePath).then(onOpen) }, filePath))]), h("div", { className: "dpp-section-title" }, h("strong", null, `Git 工作区 ${status.branch ? `· ${status.branch}` : ''}`), h("button", { onClick: refresh }, "刷新"), h("button", { onClick: snapshot }, "记录快照")), ...status.changes.map((item) => h("div", { className: "dpp-change", key: item.path }, h("button", { onClick: () => diff(item) }, h("span", { className: "dpp-status" }, `${item.index}${item.worktree}`), item.path), h("button", { className: "dpp-danger", onClick: () => discard(item) }, "撤销"))), h("div", { className: "dpp-section-title" }, h("strong", null, "修改快照")), ...history.map((item) => h("div", { className: "dpp-history", key: item.id }, h("div", null, h("strong", null, item.turn ? `回复 ${item.turn}` : item.label), h("small", null, new Date(item.createdAt).toLocaleString())), h("button", { onClick: () => revert(item) }, "恢复"))));
    }
