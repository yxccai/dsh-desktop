window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PluginCenterTab() {
      ...
      const refresh = async () => { try { const data = await api.pluginCenter.list(); setState({ loading: false, data, error: "" }); const custom = (data.recommended || []).find((entry) => entry.id === CUSTOM_BACKGROUND_ID); setBackground(custom?.status === 'enabled' ? await api.pluginCenter.backgroundGet() : null); } catch (error) { setState({ loading: false, data: null, error: error?.message || String(error) }); } };
