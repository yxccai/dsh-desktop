  ...
  factory: (require) => {
    ...
    function PluginCenterTab() {
      ...
      const marketAct = async (action) => {
        ...
        if (action === 'uninstall' && !window.confirm('确认卸载插件市场？卸载后，浏览器 DSH Web 和 DSH Desktop 中将不再显示插件市场入口。')) return;
        setMarketBusy(true);
      ...
      const marketButtons = (() => {
        ...
        if (entry.status === 'available') return [item('安装插件市场', 'install', false, true)];
        if (!entry.owned) return [];
        if (entry.status === 'enabled') return [item('停用插件市场', 'disable'), item('卸载插件市场', 'uninstall', true)];
        if (entry.status === 'disabled') return [item('启用插件市场', 'enable', false, true), item('卸载插件市场', 'uninstall', true)];
        return [];
      })();
      const marketCard = market ? h('section', { className: 'dpc-market' },
        h('div', { className: 'dpc-market-head' },
          h('div', null, h('h3', null, 'Web 插件市场（可选）'), h('p', null, '浏览 awesome-dsh-plugin.com 上的社区插件。安装后，这些插件会同时出现在浏览器 DSH Web 和 DSH Desktop 中。'))),
        ...
        market.entry ? h('p', { className: 'dpc-note' }, '更改后重启 DSH 即可生效。') : null) : null;
      ...
      const notificationRow = notifications.unsupported ? null : h('section', { className: 'dpc-note dpc-notification-row' },
        h('div', { className: 'dpc-notification-text' },
          h('strong', null, 'Agent 系统通知'),
          h('p', null, '有审批请求/任务完成/报错时弹 Windows 通知'),
          notifications.error ? h('small', { className: 'dpc-error' }, notifications.error) : null),
        h('input', { className: 'dpc-notification-toggle', type: 'checkbox', checked: notifications.enabled, disabled: notifications.loading, onChange: (event) => setNotificationsEnabled(event.target.checked) }));
