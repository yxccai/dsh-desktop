window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ProjectPanel({ useSessions, useWorkspaces }) {
      ...
      const startDrag = (event) => { event.preventDefault(); const startX = event.clientX, startWidth = width; const move = (e) => { e.preventDefault(); setWidth(Math.max(320, Math.min(1000, startWidth + startX - e.clientX))); }; const up = (e) => { const next = Math.max(320, Math.min(1000, startWidth + startX - e.clientX)); setWidth(next); api.projectPanel.setState(root, { width: next, collapsed: false, activeTab: tab }); document.body.style.cursor = ''; document.body.style.userSelect = ''; window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', up); }; document.body.style.cursor = 'col-resize'; document.body.style.userSelect = 'none'; window.addEventListener('pointermove', move); window.addEventListener('pointerup', up); };
    ...
    function mountProjectColumn() {
      let frame = null; let column = null; let observer = null; let shellTracks = [];
      let panelWidth = 0;
      const applyGrid = () => { if (frame && shellTracks.length === 3) frame.style.gridTemplateColumns = `${shellTracks[0]} minmax(0, 1fr) ${shellTracks[2]} ${Math.round(panelWidth)}px`; if (column) column.style.visibility = panelWidth > 0 ? 'visible' : 'hidden'; };
      const attach = () => {
        if (frame) return;
      ...
      return () => { wait.disconnect(); observer?.disconnect(); if (frame && shellTracks.length === 3) frame.style.gridTemplateColumns = shellTracks.join(' '); column?.remove(); projectPanelColumn = null; projectPanelLayout = null; };
    }
