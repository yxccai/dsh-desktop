window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ConversationFiles({ matched, useSessions, sessionId }) {
      ...
      React.useEffect(() => {
        if (!root || !matched?.length) return;
        let cancelled = false;
        const frame = requestAnimationFrame(() => requestAnimationFrame(() => { if (!cancelled) window.dispatchEvent(new CustomEvent("dsh-desktop-project-file", { detail: { root, path: matched[matched.length - 1] } })); }));
